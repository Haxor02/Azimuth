...
namespace Azimuth
{
	public class Config
	{
		...
		{
			// ?? = if null, assign. If not null, do nothing
			instance ??= new Config();
		}
		...
		public static VALUE? Get<VALUE>(string _category, string _key)
		{
			...
			Console.WriteLine($"[Error] Attempted to get config value for type '{_key}' in '{_category}'.");
		...
		private void Load()
		{
			...
			// Somehow the file has no directory... WAT?
			if(file.DirectoryName == null)
				return;

			// If the directory doesn't exist, create it, and return
